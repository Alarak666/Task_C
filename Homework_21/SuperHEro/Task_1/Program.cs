...
namespace Task_1
{
    public class Program
    {
        ...
        public class SuperHero
        {
            ...
            public SuperHero Continue(ref SuperHero super)// НОрмально
            {
                using (FileStream stream = new FileStream("Binary.xml", FileMode.OpenOrCreate))
                {
                    BinaryFormatter binary = new BinaryFormatter();
            ...
            public void SaveB(ref SuperHero super)// бинарно
            {
                using (FileStream stream = new FileStream("Binary.xml", FileMode.OpenOrCreate))
                {
                    BinaryFormatter binary = new BinaryFormatter();
            ...
            public SuperHero ContinueB(ref SuperHero super)// бинарно
            {
                using (FileStream stream = new FileStream("Binary.xml", FileMode.OpenOrCreate))
                {
                    BinaryFormatter binary = new BinaryFormatter();
            ...
            public SuperHero Otver(ref SuperHero super, int variant)
            {
                if (variant == 1)
                {
                    super.score += 10;
        ...
        public class Pistol
       {
        ...
        static void Main(string[] args)
        {
            ...
            SuperHero ANTON = new SuperHero { name = "Anton", pistol = new Pistol { name = "M16", damage = 12f }, score = 0, hp = 100, quests = quests};
            ...
            Console.WriteLine("Загрузить игру? = 4, Сохранить игру = 3");
            ...
            do
            {
                do
                {
                    ...
                    if (variant == 3)
                    {
                        ...
                        if (variant == 1)
                        {
                            ANTON.SaveB(ref ANTON);
                        }
                        else
                        {
                            ANTON.Save(ref ANTON);
                        }
                    }
                    variant = int.Parse(Console.ReadLine());
                } while (variant > 2 );
